...
% +~~~~~~~~~~~~~~~~~~~~~~~~~~~~+
\subsection{Install \texttt{easydb}} 
...
<<echo=TRUE,eval=TRUE>>= 
# Get the file path:
sqliteFile <- system.file( "soils.db", package = "easydb" ) 
sqliteFile 
#
# Describe the database:
sDb <- edb( dbName = sqliteFile, dbType = "RSQLite_SQLite" ) 
@ 
...
\begin{itemize}
    ...
    \item \texttt{dbType}: Database type. Here the string 
        \texttt{RSQLite_SQLite} should be understood as 
...
<<echo=TRUE,eval=TRUE>>= 
# Get the file path:
accessFile <- system.file( "soils.mdb", package = "easydb" ) 
accessFile 
#
# Describe the database:
aDb <- edb( dbName = access, dbType = "RSQLite_SQLite" ) 
@ 
...
\begin{itemize}
    \item \texttt{dbName}: Name and path of the database, as a text 
        string;
    \item \texttt{dbType}: Database type. Here the string 
        \texttt{RODBC_Access} should be understood as 
...
that this functions is experimental (be careful):
...
% +~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~+
\subsection{Second step: 'inspect' you database} 
