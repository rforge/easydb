...
\title{\texttt{easydb}, easy and simple database interface for%
...
(single brackets \[ \] subsetting). Nonetheless it does not work 
...
\texttt{easydb} is licensed under an Affero GNU General Public 
License Version 3 (\texttt{http://www.gnu.org/licenses/agpl.html}).\\
...
or his employer} (Swedish University of Agricultural Sciences).\\

Many ideas behind \texttt{easydb} have been introduced before by 
\textbf{John Fox and Oswaldo Cruz} in their package \texttt{dfdb} 
(\texttt{http://r-forge.r-project.org/projects/dfdb-rodbc/}), in 
...
The author of this package is neither a database specialist (rather a 
